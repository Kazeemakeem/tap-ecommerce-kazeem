...
const ProductCard = ({
  ...
  const wishListIds = useAppSelector((state) => state.wishlist.items).map(
    (item) => item._id
  );
  ...
  const wishlistHandler = () => {
    !wishlisted &&
      dispatch(
        addToWishlist({ _id, name, description, discount, price, rating })
      );
    wishlisted && dispatch(removeFromWishlist(_id));
  };
  ...
  return (
    <TouchableOpacity className="flex-row space-x-4 py-4 border-b-2 border-gray-300">
      <View className="w-36 h-48 rounded-lg border-2 border-gray-200 flex items-center justify-center">
        <View className={`w-24 h-24 bg-[#fab005] rounded-full`}></View>
      </View>
      <View className="flex justify-between pb-8 pt-2">
        <Text className="text-lg">{name}</Text>
        <View className="bg-red-100 w-20 flex items-center rounded-sm">
          <Text testID="discount" className="text-red-500 p-1">
            | {discount} % OFF |
          </Text>
        </View>
        <Text testID="item-price" className="text-lg">{valueFormattedWithSymbol}</Text>
        <View>
          <AirbnbRating
            count={5}
      ...
      <TouchableOpacity
        ...
        className="absolute -left-4 top-4 w-10 h-10 flex items-center justify-center rounded-br-xl rounded-tl-xl bg-gray-400"
        onPress={wishlistHandler}
      >
        {wishlisted ? (
          <HeartSolid size={30} color={colors.primary} />
        ) : (
