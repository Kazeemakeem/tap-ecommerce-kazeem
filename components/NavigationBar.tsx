...
const NavigationBar = ({ routeName }: NavigationBarProps) => {
  ...
  const navButtonWithCounter = (Icon: any, text: string, count: number) => (
    ...
    >
      ...
      <View className="flex items-center justify-center bg-white w-4 h-4 absolute bottom-6 left-6 rounded-full">
        <Text className="text-sm text-red-500 font-bold">{count}</Text>
      </View>
    </TouchableOpacity>
  );
