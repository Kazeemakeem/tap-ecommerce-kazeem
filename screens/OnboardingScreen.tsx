...
import { Dimensions, SafeAreaView, ScrollView, View, TouchableOpacity, 
          Text, NativeSyntheticEvent, NativeScrollEvent } from 'react-native'
...
const OnboardingSwipeScreen = () => {
  ...
  const boardButton = (text: string, slideIndex: number) => (
    ...
    className={`w-full px-4 absolute bottom-2 flex flex-row ${slideIndex === 0 ? "justify-start" : slideIndex === 2 ? "justify-end" : ""}`} 
      ...
      ><View className="bg-[#fab005] flex justify-center items-center rounded-lg">
    ...
    <SafeAreaView className="flex-1">
      ...
      <View className="absolute bottom-20 left-0 right-0 justify-center mr-3 items-center flex-row">
        {Array.from(Array(3).keys()).map((key, index) => (
          <View key={index} className={`h-3 w-3 ml-3 rounded-full ${pageIndex === index ? "bg-[#fab005]" : "bg-white"}`} />
        ))}
      </View>
